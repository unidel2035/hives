#!/usr/bin/env python3
"""
Исправленная полная LZP декомпиляция PAC файла
...
class LZPDecompressor:
    ...
    def patternreplace(self, s: str, lzpmask: bool) -> str:
        ...
        if lzpmask:
            ...
            patterns = {
                ...
                '!~': 'ture', '![': 'tech', '!]': 'ance', '!{': 'coin',
                '!}': 'seed', '!`': 'anim', '!:': 'stro', '!;': 'ment',
                '!?': 'site', 'A': 'in', 'B': 'an', 'C': 'er', 'D': 'ar',
                'E': 'or', 'F': 'et', 'G': 'al', 'H': 'st', 'I': 'on',
                'J': 'en', 'K': 'at', 'L': 'ro', 'M': 'es', 'N': 'as',
                'O': 'el', 'P': 'it', 'Q': 'ch', 'R': 'am', 'S': 'ol',
                'T': 'om', 'U': 'ra', 'V': 'ex', 'W': 'is', 'X': 'ic',
                'Y': 're', 'Z': 'os', '@': 'ka', '#': 'ot', '$': 'us',
                '%': 'ap', '^': 'ov', '&': 'im', '*': '-s', '(': 'ad',
                ')': 'il', '=': 'op', '+': 'ed', '/': 'em', ',': 'a-',
                '<': 'od', '>': 'ir', '~': 'id', '[': 'ob', ']': 'ag',
    ...
    def unlzp_function(self, d: str, m: str, lim: int) -> Tuple[str, int, int]:
        ...
        # Сброс таблицы для каждого вызова
        self.table = [0] * 262144
...
class CompletePACDecompiler:
    ...
    def extract_ip_list(self):
        ...
        try:
            # Находим список IP адресов - исправленное регулярное выражение
            ...
            for zone, domain_dict in domains_copy.items():
                ...
                for length, count in domain_dict.items():
                    if isinstance(count, int) and count > 0:
                        ...
                        reqd = min(count * 20, 8192)  # Увеличиваем буфер
...
def main():
    if len(sys.argv) < 2:
        print("Использование: python lzp_decompiler_fixed.py <pac_file>")
        print("Пример: python lzp_decompiler_fixed.py pac.pac")
        sys.exit(1)
